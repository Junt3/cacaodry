...
def comofunciona():
    """Página que explica las fórmulas y papers científicos utilizados"""
...
def ssh_connect():
    ...
    if not hostname:
        return jsonify({'success': False, 'error': 'El hostname es requerido'}), 400
    if not username:
    ...
    try:
        ...
        if port < 1 or port > 65535:
            return jsonify({'success': False, 'error': 'Puerto inválido'}), 400
    except ValueError:
        return jsonify({'success': False, 'error': 'Puerto debe ser un número'}), 400

    # Crear ID de sesión único basado en la sesión Flask
    session_id = session.get('config_session_id') or f"ssh_{session.sid if hasattr(session, 'sid') else id(session)}"

    # Crear nueva sesión SSH
    ssh_session = ssh_manager.create_session(session_id)

    # Conectar
    success, message = ssh_session.connect(hostname, username, password, port)
...
def ssh_disconnect():
    ...
    return jsonify({'success': True, 'message': 'Desconectado correctamente'})


@app.route('/api/ssh/status', methods=['GET'])
@requiere_autenticacion_config
def ssh_status():
    """Obtiene el estado de la conexión SSH"""
    session_id = session.get('config_session_id') or f"ssh_{session.sid if hasattr(session, 'sid') else id(session)}"
...
def ssh_execute():
    ...
    ssh_session = ssh_manager.get_session(session_id)

    if not ssh_session:
        return jsonify({'success': False, 'error': 'No hay sesión SSH activa'}), 403

    success, output = ssh_session.execute_command(command)
...
@app.route('/api/ssh/output', methods=['GET'])
@requiere_autenticacion_config
def ssh_output():
    """Obtiene la salida de la sesión SSH"""
    session_id = session.get('config_session_id') or f"ssh_{session.sid if hasattr(session, 'sid') else id(session)}"
    ...
    return jsonify({
        'success': True,
        'output': output,
        'connected': ssh_session.is_active()
    })
...
@requiere_autenticacion_config
def ssh_send_key():
    """Envía una tecla especial a la sesión SSH"""
    data = request.get_json()
    ...
    success = ssh_session.send_special_key(key)

    # Leer salida después de enviar la tecla
    time.sleep(0.1)
    output = ssh_session.get_output()
